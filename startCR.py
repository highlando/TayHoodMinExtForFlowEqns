...
#from math import log as ln				# erlaubt ln(.)
...
#     print 'Koordinate (%3g, %3g) mit Wert %6g' % (coord[i][0], coord[i][1], u_array[i])
...
class ProbParams(object):
	def __init__(self,N,Omega):
		self.mesh = UnitSquareMesh(N, N) #smartminex_tayhoomesh.getmake_mesh(N)
		self.N = N
		self.V = VectorFunctionSpace(self.mesh, "CR", 1)		# CR-P0 oder VectorFunctionSpace
		self.Q = FunctionSpace(self.mesh, "DG", 0)			# nur scalar (daher ohne Vector)	
		self.velbcs = setget_velbcs_zerosq(self.mesh, self.V)		# define Dirichlet boundary
		...
		for bc in self.velbcs:
			bcdict = bc.get_boundary_values()		# dictionary fuer boundary(knoten_ind-value)
			bcinds.extend(bcdict.keys())			# nur die indices in Liste speichern

		# indices of the inner velocity nodes
		self.invinds = np.setdiff1d(range(self.V.dim()),bcinds) # indices der inneren Knoten
...
# FERTIG!  
## returns an array which couples the index of an edge with the DoFs of the velocity
...
def computeEdgeCRDofArray(V, mesh):  
  ...
  for cell in cells(mesh):       
    dofs = dofmap.cell_dofs(cell.index())                     	# list of dof-indices for edges of the cell    
    for i, facet in enumerate(facets(cell)):
      #print 'cell: %3g  ||  i: %3g   || facet: %3g' % (cell.index(), i, facet.index())            
      edgeCRDofArray[facet.index()] = [dofs[i], dofs[i]+1]	# corresponding DoFs (2 basisfct per edge)	
...
#  given a cell and a mesh, it returns the indices of all cells which share a common edge with the given cell
def findAdjacentCellInd(cell, mesh):
  ...
  #adj_cells = np.delete(adj_cells, dieser_index)		# np.delete(array, position_to_delete)  
...
def computeSmartMinExtMapping(V, mesh):
  ...
  while (len(R) > 0):    
    ...
    adm_adj_cells = np.intersect1d(adj_cells_last_T, R)		# only adjacent cells which are also in R
    ...
    E = np.append(E, comm_edge)					# update range(iota), i.e., list of edges
...
def get_SmartDoFs_CR(V, mesh, B_matrix):  
  ...
  # we still have do decide which of the two basis functions corresponding to the edge we take
  # here: take as default [phi_E; 0] if not div[phi_E; 0] = 0 (check the column in B_matrix)
 #  ??? ist pressure-DoF von B_matrix schon entfernt?
  ...
  for i in np.arange(len(edges_V2)):
    ...
    dof = DoF_for_V2_x[i]
    col = Ba[:, dof]
    if LA.norm(col.toarray(), np.inf) < 1e-13:
...
#  solve_stokesTimeDep()
  ...
  #print 'NumOfNodes %3g and dimension %3g' % (PrP.mesh.num_vertices(), len(u_array))
  print ' ------------ '
  ...
  print 'Die Determinante ist = %3g' % LA.det(reg_B2_bloc.toarray())
